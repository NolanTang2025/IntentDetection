...
def load_clustering_results(shop_id="53"):
    """加载聚类结果文件"""
    clustering_file = BASE_DIR / 'cluster_results_by_shop' / f'shop_{shop_id}' / f'clustering_results_{shop_id}.json'
    if not clustering_file.exists():
        print(f"❌ 错误: 找不到聚类结果文件 {clustering_file}")
        return None
    
    with open(clustering_file, 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    # 支持两种格式：直接是对象，或者有shop_id作为key
    if shop_id in data:
        return data[shop_id]
    else:
        return data

def load_extracted_data(shop_id="53"):
    """加载原始提取的数据"""
    extracted_file = DATA_EXTRACT_DIR / f'extracted_data_shop_{shop_id}.json'
    if not extracted_file.exists():
        print(f"❌ 错误: 找不到原始数据文件 {extracted_file}")
        return None
    
    with open(extracted_file, 'r', encoding='utf-8') as f:
        return json.load(f)

def convert_to_business_cluster_format(clustering_data, extracted_data, shop_id="53"):
...
def main():
    ...
    print("=" * 80)
    print(f"处理 Shop {shop_id} 聚类结果")
    print("=" * 80)
    
    # 步骤1: 加载数据
    print("\n[1/5] 加载聚类结果和原始数据...")
    clustering_data = load_clustering_results(shop_id)
    if not clustering_data:
        return
    
    extracted_data = load_extracted_data(shop_id)
    if not extracted_data:
        return
    
    print(f"  ✓ 聚类结果: {clustering_data.get('n_intentions', 0)} 个意图, {clustering_data['clustering']['n_clusters']} 个聚类")
    print(f"  ✓ 原始数据: {len(extracted_data)} 条记录")
    
    # 步骤2: 转换为business_cluster_results格式
    print("\n[2/5] 转换数据格式...")
    business_cluster_results = convert_to_business_cluster_format(clustering_data, extracted_data, shop_id)
    ...
    # 步骤2.5: 使用embedding-based标签生成器生成聚类标签
    print("\n[2.5/5] 生成基于文本的聚类标签...")
    ...
    # 更新business_cluster_results中的cluster_labels
    business_cluster_results['clustering']['cluster_labels'] = cluster_labels_dict
    print(f"  ✓ 已生成 {len(cluster_labels_dict)} 个聚类的标签")
    
    # 步骤3: 保存business_cluster_results
    print("\n[3/5] 保存聚类结果...")
    output_file = CLUSTER_DIR / f'business_cluster_results_shop_{shop_id}.json'
    ...
    try:
        ...
        analyzer = BusinessDrivenPortraitAnalyzer(
            cluster_results_file=f'../cluster_timeClip/business_cluster_results_shop_{shop_id}.json'
        )
        strategies = analyzer.analyze_all_clusters()
        analyzer.save_results(strategies, output_dir=PORTRAIT_DIR)
    ...
    # 生成data_shop_53.js
    output_js = DASHBOARD_DIR / f'data_shop_{shop_id}.js'
    with open(output_js, 'w', encoding='utf-8') as f:
        f.write(f'// Shop {shop_id} 聚类分析数据\n')
        f.write('// 自动生成，请勿手动编辑\n\n')
